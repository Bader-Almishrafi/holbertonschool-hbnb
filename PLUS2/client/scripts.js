...
function hydrateNavigation() {
  ...
  fetchMe()
    .then((me) => {
      ...
      const isAdmin = user.is_admin === true || me.is_admin === true;

      if (adminLink) adminLink.classList.toggle('d-none', !isAdmin);

      if (authLink && user.full_name) {
        authLink.textContent = user.full_name;
      }
    })
    .catch(() => {
      clearSession();
        ...
        <div class="card-body d-flex flex-column">
          <div class="d-flex justify-content-between align-items-start gap-2 mb-2">
            ...
            <span class="badge rounded-pill rating-badge">
              <i class="bi bi-star-fill"></i> ${place.average_rating ?? 'New'}
            </span>
          </div>
          <p class="text-muted">${place.description || 'No description available.'}</p>
          <div class="mt-auto d-flex justify-content-between align-items-center">
            <div>
              <strong>${fmtMoney(place.price)}</strong>
    ...
    try {
      const data = await api('/auth/register', {
        ...
        body: JSON.stringify({
          ...
          email: qs('register_email').value.trim().toLowerCase(),
          password: qs('register_password').value
        })
      });
...
async function initPlacePage() {
  ...
  details.innerHTML = `
    <div class="row g-4">
      <div class="col-lg-7">
        <img src="${place.image_url || 'https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?auto=format&fit=crop&w=1400&q=80'}" class="img-fluid rounded-4 shadow-sm place-cover w-100" alt="${place.title}">
      </div>
        ...
        <div class="card border-0 shadow-sm rounded-4 h-100">
          <div class="card-body p-4">
            <div class="d-flex justify-content-between align-items-start mb-3">
              <div>
                <h1 class="h2 fw-bold mb-1">${place.title}</h1>
                <p class="text-muted mb-0">${place.city}, ${place.country} • Hosted by ${place.owner_name}</p>
              </div>
              <span class="badge rating-badge rounded-pill">
                <i class="bi bi-star-fill"></i> ${place.average_rating ?? 'New'}
              </span>
            </div>
            ...
            <div class="row g-3 small mb-4">
              <div class="col-6">
                <div class="p-3 bg-light rounded-3">
                  <strong>${fmtMoney(place.price)}</strong><br>
                  <span class="text-muted">per night</span>
                </div>
              </div>
              <div class="col-6">
                <div class="p-3 bg-light rounded-3">
                  <strong>${place.max_guests}</strong><br>
            ...
            <div class="d-flex flex-wrap gap-2 mb-4">
              ${
                place.amenities?.length
                  ? place.amenities.map((a) => `<span class="badge text-bg-light border">${a.name}</span>`).join('')
              ...
              isLoggedIn
                ? `
                  <form id="booking-form" class="border rounded-4 p-3 bg-light-subtle">
                    ...
                    <div class="row g-2">
                      <div class="col-6">
                        ...
                        <input id="check_in_date" type="date" class="form-control" required>
                      </div>

                      <div class="col-6">
                        <label class="form-label small">Check out</label>
                        <input id="check_out_date" type="date" class="form-control" required>
                      </div>

                      <div class="col-12">
                        <label class="form-label small">Guests</label>
                        <input id="booking_guests" type="number" min="1" max="${place.max_guests}" value="1" class="form-control" required>
                      </div>
                    </div>

                    <button class="btn btn-danger w-100 mt-3">Book now</button>
                    <div id="booking-message" class="small mt-3"></div>
                  </form>
                  ...
                  <form id="review-form" class="border rounded-4 p-3 mt-3">
                    ...
                    <div id="review-message" class="small mt-3"></div>
                  </form>
                `
                : `<a href="login.html" class="btn btn-danger w-100">Login to book or review</a>`
            }
          </div>
        </div>
      </div>
    ...
    <section class="mt-5">
      <h2 class="h4 fw-bold mb-3">Guest reviews</h2>
      <div class="row g-3">
        ${
                  ...
                  <div class="card border-0 shadow-sm h-100">
                    <div class="card-body">
                      <div class="d-flex justify-content-between">
                        ...
                        <span class="badge rating-badge rounded-pill">${review.rating}/5</span>
                      </div>
                      <p class="text-muted small mt-2 mb-0">${review.text}</p>
                    </div>
                  </div>
                </div>
              `).join('')
  ...
  qs('booking-form')?.addEventListener('submit', async (e) => {
    ...
    try {
      await api('/bookings/', {
        ...
        body: JSON.stringify({
          place_id: place.id,
          check_in_date: qs('check_in_date').value,
          check_out_date: qs('check_out_date').value,
          guests: Number(qs('booking_guests').value)
        })
      });
  ...
  qs('review-form')?.addEventListener('submit', async (e) => {
    ...
    try {
      await api('/reviews/', {
        ...
        body: JSON.stringify({
          place_id: place.id,
          text: qs('review-text').value.trim(),
    ...
    <div class="col-lg-6">
      <div class="card border-0 shadow-sm rounded-4 h-100">
        <div class="card-body">
          <div class="d-flex justify-content-between align-items-start mb-3">
            <div>
              <h2 class="h5 fw-bold mb-1">${booking.place_title}</h2>
              <p class="text-muted small mb-0">${booking.check_in_date} → ${booking.check_out_date}</p>
            </div>
          ...
          <p class="mb-2"><strong>Total:</strong> ${fmtMoney(booking.total_price)}</p>
          <p class="mb-3"><strong>Guests:</strong> ${booking.guests}</p>

          ${
            booking.status !== 'cancelled'
              ? `<button class="btn btn-outline-danger" onclick="cancelBooking('${booking.id}')">Cancel booking</button>`
              : ''
          }
        </div>
      </div>
...
async function initMyPlacesPage() {
  ...
  renderMyPlaces(allPlaces.filter((place) => place.owner_id === user.id));
  ...
  qs('place-form')?.addEventListener('submit', async (e) => {
    ...
    try {
      await api('/places/', {
        method: 'POST',
        body: JSON.stringify({
          title: qs('place_title').value.trim(),
          description: qs('place_description').value.trim(),
          price: Number(qs('place_price').value),
          max_guests: Number(qs('place_guests').value),
          city: qs('place_city').value.trim(),
          country: qs('place_country').value.trim(),
          latitude: Number(qs('place_latitude').value),
          longitude: Number(qs('place_longitude').value),
          image_url: qs('place_image_url').value.trim(),
          amenities: []
        })
      });

      msg.className = 'small text-success mt-3';
      msg.textContent = 'Place created successfully.';
...
function renderMyPlaces(places) {
  const list = qs('my-places-list');
  if (!list) return;

  list.innerHTML = places.map((place) => `
    <div class="col-md-6">
      <div class="card border-0 shadow-sm rounded-4 overflow-hidden h-100">
        <img src="${place.image_url || 'https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?auto=format&fit=crop&w=1200&q=80'}" class="card-img-top place-card-image" alt="${place.title}">
        <div class="card-body">
          <h3 class="h5 fw-bold">${place.title}</h3>
          <p class="text-muted small">${place.city}, ${place.country}</p>
          <p class="text-muted">${place.description || 'No description available.'}</p>

          <div class="d-flex justify-content-between align-items-center">
            <span><strong>${fmtMoney(place.price)}</strong> / night</span>
            <a class="btn btn-outline-danger btn-sm" href="place.html?id=${place.id}">View</a>
          </div>
        </div>
      </div>
    </div>
...
async function initAdminPage() {
  ...
  const [stats, users, bookings, places] = await Promise.all([
    api('/admin/stats'),
    api('/admin/users'),
    api('/admin/bookings'),
    api('/admin/places')
  ...
  const usersTable = qs('admin-users-table');
  ...
  const bookingsTable = qs('admin-bookings-table');
...
function renderAdminCharts(bookings, places) {
  ...
  const statusChart = qs('booking-status-chart');
  const revenueChart = qs('monthly-revenue-chart');
  const citiesChart = qs('top-cities-chart');

  if (!statusChart || !revenueChart || !citiesChart) return;
  ...
  new Chart(revenueChart, {
    ...
    data: {
      ...
      datasets: [
        {
          label: 'Revenue',
          data: revenueLabels.map((label) => Math.round(revenueByMonth[label]))
        }
      ]
    },
    options: {
