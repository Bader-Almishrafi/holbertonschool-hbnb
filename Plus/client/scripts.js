...
async function initAdminPage() {
  ...
  const [stats, users, bookings] = await Promise.all([api('/admin/stats'), api('/admin/users'), api('/admin/bookings')]);
  ...
  qs('admin-users-table').innerHTML = `<thead><tr><th>Name</th><th>Email</th><th>Role</th></tr></thead><tbody>${users.slice(0, 8).map((user) => `<tr><td>${user.full_name}</td><td>${user.email}</td><td>${user.is_admin ? 'Admin' : 'User'}</td></tr>`).join('')}</tbody>`;
  qs('admin-bookings-table').innerHTML = `<thead><tr><th>Guest</th><th>Place</th><th>Status</th></tr></thead><tbody>${bookings.slice(0, 8).map((booking) => `<tr><td>${booking.user_name}</td><td>${booking.place_title}</td><td>${booking.status}</td></tr>`).join('')}</tbody>`;
}
